 ...
 *
 * A Map rather than a plain object because the key is whatever the customer
 * typed. Object lookup walks the prototype chain, so `SYNONYMS["constructor"]`
 * returned a function rather than undefined, `?? []` did not catch it because
 * a function is not nullish, and iterating it threw. Searching the word
 * "constructor" took the search out. A Map has no prototype keys to collide
 * with, so the whole class of that bug goes away rather than being guarded
 * against at the one call site that happened to hit it.
 */
...
export function searchCatalogue(query: string, services?: Service[]): SearchResult[] {
  ...
  for (const token of tokens) {
    for (const id of SYNONYMS.get(token) ?? []) {
